...
export default function RevealScreen() {
  ...
  const topIds = Object.keys(tally).filter(
    (id) => tally[id] === maxVotes && maxVotes > 0
  );

  const semPlayerId = players[semIndex]?.id;

  function handleSentToGuess() {
    navigate("/guess");
  }
          ...
          return (
            <div
              ...
              style={{
                padding: 12,
                borderRadius: 10,
                border: isTop ? "2px solid #2f80ed" : "1px solid #eee",
                background: "#fff",
                display: "flex",
            ...
            >
              ...
              <div style={{ textAlign: "right" }}>
                {isTop ? (
                  <div style={{ color: "#2f80ed", fontWeight: 800 }}>
                    {c} votos — mais votado
                  </div>
                ) : (
                  <div style={{ color: "#444" }}>{c} votos</div>
                )}
              </div>
            </div>
          );
